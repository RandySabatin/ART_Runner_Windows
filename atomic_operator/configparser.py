...
class ConfigParser(Base):
    ...
    def __build_run_list(self, techniques=None, test_guids=None, host_list=None, select_tests=False):
        ...
        self.__logger.info("__LOADED_TECHNIQUES: " + str(self.__loaded_techniques))
        self.__logger.info("TEST_GUIDS: " + str(test_guids))
        if test_guids:
            for key,val in self.__loaded_techniques.items():
                test_list = []
                self.__logger.info("TEST_key: " + str(key))
                for test in val.atomic_tests:
                    self.__logger.info("TEST_test.auto_generated_guid: " + str(test.auto_generated_guid))
        ...
        if techniques:
            if 'all' not in techniques:
                ...
                pass
        self.__logger.info("__RUN_LIST: " + str(__run_list))                
        return __run_list
